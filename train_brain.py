...
def fetch_recent_data():
    """Downloads the last 14 days of pitch-by-pitch Statcast data to prevent Amnesia."""
    ...
    try:
        ...
        if df is None or df.empty:
            print("No data found for the recent window. Games may not have been played.")
            return None
        return df
...
def engineer_features(df):
    ...
    feature_cols = ['launch_speed', 'launch_angle', 'release_speed', 'is_hard_hit', 'is_barrel', 'is_blast',
                    'hr_park_factor']
...
def update_brain():
    ...
    # 1. Get Recent Data (Now fetching a 14-day window instead of 1 day)
    ...
    print(f"Training gently on {len(X_new)} recent batted ball events...")
    ...
    params = {
        ...
        "eval_metric": "logloss",
        "learning_rate": 0.03,  # FIX: Lowered from 0.05 so it doesn't overreact to recent games
        "max_depth": 6,
        "subsample": 0.8,
        "colsample_bytree": 0.8,
    }

    # FIX: Changed num_boost_round from 10 to 2 to prevent the AI from bloating with useless trees
    ...
    print(f"[SUCCESS] The Home Run Brain has successfully updated its weights using the last 14 days of games.")
