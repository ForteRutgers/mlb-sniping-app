...
def fix_savant_name(name_string):
...
def fetch_best_available_data():
    """Bypasses blocked providers and goes straight to official Statcast data."""
    print("      -> Bypassing blocked sites, connecting directly to Statcast...")
    try:
        from pybaseball import statcast_batter_expected_stats, statcast_pitcher_expected_stats
        import warnings
        warnings.filterwarnings('ignore')

        # Pull 2024 data to ensure we have a full season of plate appearances to base our math on
        b = statcast_batter_expected_stats(2024, 50)
        p = statcast_pitcher_expected_stats(2024, 50)

        if b is not None and not b.empty:
            b.columns = b.columns.str.upper()
            p.columns = p.columns.str.upper()

            # --- THE NAME FLIPPER FIX ---
            if 'LAST_NAME, FIRST_NAME' in b.columns:
                b['NAME'] = b['LAST_NAME, FIRST_NAME'].apply(fix_savant_name)
            elif 'FIRST_NAME' in b.columns and 'LAST_NAME' in b.columns:
                b['NAME'] = b['FIRST_NAME'] + ' ' + b['LAST_NAME']
            else:
                b['NAME'] = "Unknown"

            if 'LAST_NAME, FIRST_NAME' in p.columns:
                p['NAME'] = p['LAST_NAME, FIRST_NAME'].apply(fix_savant_name)
            elif 'FIRST_NAME' in p.columns and 'LAST_NAME' in p.columns:
                p['NAME'] = p['FIRST_NAME'] + ' ' + p['LAST_NAME']
            else:
                p['NAME'] = "Unknown"

            # Reconstruct our required ML stats from the Statcast data
            b['PA'] = get_col_safe(b, 'PA', 1)
            # FIX: Use EST_BA and EST_SLG to match the actual Statcast database names!
            b['AVG'] = get_col_safe(b, 'EST_BA', 0.240)
            b['SLG'] = get_col_safe(b, 'EST_SLG', 0.400)
            b['HR'] = (b['SLG'] - b['AVG']) * 0.15 * b['PA']
            b['H'] = b['AVG'] * b['PA']
            b['2B'] = b['H'] * 0.20
            b['3B'] = b['H'] * 0.02

            # FIX: Dynamically check for Walk and Strikeout percentages if they exist
            b['BB'] = (get_col_safe(b, 'BB_PERCENT', 8.5) / 100) * b['PA'] if 'BB_PERCENT' in b.columns else b[
                                                                                                                 'PA'] * 0.085
            b['SO'] = (get_col_safe(b, 'K_PERCENT', 22.5) / 100) * b['PA'] if 'K_PERCENT' in b.columns else b[
                                                                                                                'PA'] * 0.225
            ...
            b['RBI'] = b['H'] * 0.4
            b['BARREL%'] = 0.08
            b['XWOBA'] = get_col_safe(b, 'EST_WOBA', 0.320)
        ...
        print(f"      [!] Statcast connection error: {e}")

    # --- BULLETPROOF OFFLINE FALLBACK ---
    print("      [!] Using Offline Backup Matrix to ensure the dashboard generates safely...")
    dummy_b = pd.DataFrame([{'NAME': 'OfflineBatter', 'PA': 500, 'EST_BA': 0.240, 'EST_SLG': 0.400, 'EST_WOBA': 0.320}])
    dummy_p = pd.DataFrame(
        [{'NAME': 'OfflinePitcher', 'PA': 500, 'EST_SLG': 0.400, 'K_PERCENT': 22.0, 'BB_PERCENT': 8.0,
          'EST_BA': 0.240}])
    return dummy_b, dummy_p, "Offline Default Backup"


def get_prop_matrices():
    print("\n[1/3] Running Cascading Tank Fetcher for True DNA...")
    try:
