...
# ---------------------------------------------------------------------------
# Park Factors for Machine Learning
# ---------------------------------------------------------------------------
PARK_FACTORS = {
...
def get_mlb_games(start_date, end_date):
    """Fetches historical game data directly from the MLB Stats API."""
    print(f" -> Fetching MLB Data from {start_date} to {end_date}...")
    url = f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&startDate={start_date}&endDate={end_date}&hydrate=game(content(summary)),decisions,probablePitcher,weather"

    response = requests.get(url)
    if response.status_code != 200:
        print("[!] Error connecting to MLB API")
        return []
    ...
    for date_obj in data['dates']:
        for game in date_obj['games']:
            # Only care about finished, regular season games
            if game['status']['statusCode'] == 'F' and game['gameType'] == 'R':
                try:
                    ...
                    continue  # Skip games with corrupted data
...
def run_collection():
    print("========================================")
    print("   MLB HISTORICAL TOTALS COLLECTOR      ")
    print("========================================")

    # We will fetch the 2024 and 2025 seasons
    season_2024 = get_mlb_games("2024-03-28", "2024-09-29")
    season_2025 = get_mlb_games("2025-03-27", "2025-09-28")

    all_games = season_2024 + season_2025

    df = pd.DataFrame(all_games)
    # Remove games where pitchers weren't properly logged
    df = df[(df['away_pitcher_id'] != 0) & (df['home_pitcher_id'] != 0)]

    file_name = "totals_training_data.csv"
    df.to_csv(file_name, index=False)

    print("\n[SUCCESS] Data Collection Complete!")
    print(f" -> Saved {len(df)} historical games to {file_name}")
